...
struct TraceInst {
  /*  3: 0 */ uint32_t pc;
  /*  7: 4 */ uint32_t
    mem_addr;                 // load/store addr, or branch target if taken
...
struct TraceSanitizer {
  ...
  // Sanity checks
  size_t m_ext_ops = 0;         // dummy != 0x73 (corrupt record)
  size_t br_taken_no_target = 0; // br_taken=1 but mem_addr=0

  void
  record(const TraceInst& t) {
    total++;
    if (t.mem_op == MemLoad)
      loads++;
    else if (t.mem_op == MemStore)
      stores++;
    if (t.is_branch) {
      branches++;
      if (t.br_taken) {
    ...
    if (t.mem_op == MemNone && !t.is_branch)
      alu++;
    if (t.dst_reg != 0)
      has_dst++;
    if (t.src_reg[0] != 0)
